...
use bridge_relayer::{
    queue::TransferQueue, BridgeRelayerError, CrossChainMessage, MessageType, QueuedTransfer,
};
...
fn test_queue_transfer_below_threshold() {
    ...
    assert_eq!(
        result.err(),
        Some(BridgeRelayerError::AmountExceedsThreshold)
    );
}
...
fn test_execute_transfer_not_yet_executable() {
    ...
    assert_eq!(
        result.err(),
        Some(BridgeRelayerError::TransferNotExecutable)
    );
}
