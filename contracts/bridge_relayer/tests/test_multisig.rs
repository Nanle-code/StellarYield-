...
use bridge_relayer::{multisig::MultiSigVerifier, BridgeRelayerError, ValidatorInfo};
...
fn test_multisig_insufficient_validators() {
    ...
    assert_eq!(
        result.err(),
        Some(BridgeRelayerError::InsufficientValidators)
    );
}
...
fn test_message_hash_computation() {
    ...
    let hash3 =
        MultiSigVerifier::compute_message_hash("different_domain", message_type, &payload, nonce);
...
fn test_batch_multisig_verification() {
    ...
    let batch_data = vec![
        ...
        (
            message_hash1,
            vec![&env, signature1.clone()],
            vec![&env, validator1.clone()],
        ),
        (
            message_hash2,
            vec![&env, signature2.clone()],
            vec![&env, validator2.clone()],
        ),
    ];

    // Verify batch
    let results = MultiSigVerifier::verify_batch_multisig(&env, &batch_data, &validator_info, 1);
