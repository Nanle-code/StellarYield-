...
use bridge_relayer::{
    BridgeConfig, BridgeRelayer, BridgeRelayerError, CrossChainMessage, MerkleProof, MessageType,
    MultiSignature, QueuedTransfer, ValidatorInfo, CONTRACT_VERSION, DEFAULT_MIN_VALIDATORS,
    DEFAULT_QUEUE_THRESHOLD, DEFAULT_TIME_LOCK, MAX_QUEUE_SIZE,
};
...
impl<'a> TestBridgeRelayerClient<'a> {
    ...
    fn initialize(
        ...
        self.contract_id.invoke(
            &TestBridgeRelayer::initialize,
            admin,
            initial_validators,
            config,
        );
    ...
    fn receive_message_with_merkle_proof(
        ...
        self.contract_id
            .try_invoke(
                &TestBridgeRelayer::receive_message_with_merkle_proof,
                message,
                proof,
            )
            .unwrap()
    ...
    fn receive_message_with_multisig(
        ...
        self.contract_id
            .try_invoke(
                &TestBridgeRelayer::receive_message_with_multisig,
                message,
                multi_sig,
            )
            .unwrap()
    ...
    fn execute_queued_transfer(
        &self,
        transfer_id: &BytesN<32>,
    ) -> Result<bool, BridgeRelayerError> {
        self.contract_id
            .try_invoke(&TestBridgeRelayer::execute_queued_transfer, transfer_id)
            .unwrap()
    }
    ...
    fn get_queued_transfer(&self, transfer_id: &BytesN<32>) -> Option<QueuedTransfer> {
        self.contract_id
            .invoke(&TestBridgeRelayer::get_queued_transfer, transfer_id)
    }

    fn get_all_queued_transfers(&self) -> Vec<QueuedTransfer> {
        self.contract_id
            .invoke(&TestBridgeRelayer::get_all_queued_transfers)
    }

    fn is_message_processed(&self, message_hash: &BytesN<32>) -> bool {
        self.contract_id
            .invoke(&TestBridgeRelayer::is_message_processed, message_hash)
    }
    ...
    fn update_config(
        ...
        self.contract_id
            .try_invoke(&TestBridgeRelayer::update_config, admin, new_config)
            .unwrap()
    }
    ...
    fn add_validator(
        ...
        self.contract_id
            .try_invoke(&TestBridgeRelayer::add_validator, admin, validator, weight)
            .unwrap()
    }
    ...
    fn remove_validator(
        ...
        self.contract_id
            .try_invoke(&TestBridgeRelayer::remove_validator, admin, validator)
            .unwrap()
    }

    fn emergency_pause(&self, admin: &Address) -> Result<(), BridgeRelayerError> {
        self.contract_id
            .try_invoke(&TestBridgeRelayer::emergency_pause, admin)
            .unwrap()
    }

    fn emergency_unpause(&self, admin: &Address) -> Result<(), BridgeRelayerError> {
        self.contract_id
            .try_invoke(&TestBridgeRelayer::emergency_unpause, admin)
            .unwrap()
    }
}
